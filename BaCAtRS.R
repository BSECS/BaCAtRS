...
ui <- fluidPage(
  ...
  navbarPage("BCMtRS",
             tabPanel("Home",
                      jumbotron(paste("Hi Welcome to BaCAtRS"), paste("This is web application for Blockchain & Cryptocurrency Coin Market Analysis! Supports testing Blockchians and performing analysis on Cryptocurrency Coin Market Data."), buttonLabel = "Click Me" ),
             ...
             navbarMenu("CryptoCurrency Analysis", 
                        tabPanel("Analysis of Close Price & Market Cap",
                                 sidebarLayout(
                                   sidebarPanel(
                                     # fileInput("file2", "Choose CSV File", accept=c('text/csv', 'text/comma-separated-values', 'text/plain', '.csv')),
                                     helpText("Make Data"),
                                     radioButtons("makedata", "Select:", c("No Option", "Show Data", "Make Close",  "Logivity", "Correlations", "Top Five Coins", "Make Market Cap.")),
                        ...
                        tabPanel("CryptoCurrency TS Analysis", 
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     textInput("col1", "Start Column", value="20"),
                                     textInput("col2", "End Column", value="24"),
                                     ...
                                     helpText("Efficient Frontier is calculated for coins from 20 to 24. Upgrade to advanced version to choose coins of your interest and for 'EF Plot'.")
                                   ),
                                   mainPanel(
...
server <- function(input, output, session){
  ...
  makeclose <- reactive({
    ...
    if (input$makedata == "Show Newest Currency"){
      length.col <- colSums(!is.na(close[,-1]))
      out <- sort(length.col)[1]
  ...
  toptenanalysis <- reactive({
    ...
    if (input$tsplot == "TS Plot"){
      plot(as.ts(close[, as.numeric(input$coinname)]), col="red", ylab = "Close Prices", main = paste("Volt. Chart for ", names(close)[as.numeric(input$coinname)]))
    }
    ...
    if (input$topteninput == "CVaR"){
      out <- apply(close[, 2:10], 2, function(x) CVaR(x))
    ...
    # efficient frontier
    ind <- cat(as.numeric(input$col1), ":", as.numeric(input$col2))
    ...
    #Proportions
    weight <- data.frame(proportion = weights[ind.min,], row.names = colnames(close.ef))
    ...
    if (input$tsplot == "EF Plot"){
      plot(100*sdP, 100*muP, type="l", xlim=c(0,250), ylim=c(0,35), main="Efficient Frontier", ylab="Expected Return(%)", xlab="Standard deviation(%)")
    }
